...
const stepOneSchema = z
  ...
  .refine(
    (data) => new Date(data.endDateForReleaseOfNote) > new Date(data.endDate),
    {
...
export const stepsFormSchema = z
  ...
  .refine(
    (data) => {
      ...
      const step1NoteReleaseDate = new Date(
        data.steps[0].endDateForReleaseOfNote,
      );
      const step2StartDate = new Date(data.steps[1].startDate);

      return (
        step2StartDate > step1EndDate && step2StartDate > step1NoteReleaseDate
      );
    },
    {
      message:
        "O início da Etapa 2 deve ocorrer após o fim e a liberação da nota da Etapa 1.",
...
export const useStepsForm = (editionId?: string) => {
        ...
        steps: [
          {
            ...
            endDateForReleaseOfNote: formatDateForInput(
              initialSteps[0].endDateForReleaseOfNote,
            ),
          },
          {
            ...initialSteps[1],
  ...
  const { mutate: saveMutation, isPending: isSaving } = useMutation({
    mutationFn: (createSteps: CreateStepDTO[]) =>
      saveStepsForEdition(editionId!, createSteps),
    onSuccess: (savedData) => {
      if (savedData?.steps && savedData.steps.length >= 2) {
        const stepsToReset: [Step, Step] = [
          savedData.steps[0],
          savedData.steps[1],
        ];
