...
import { findAllEnrollments, approveEnrollmentStudent, refuseEnrollmentStudent } from "../services/enrollmentStudentService";
...
export const useEnrollmentStudentTable = (editionYear: string) => {
  ...
  const queryKey = ['studentEnrollments', editionYear, { pageIndex, pageSize, filter: debouncedFilter, sort }];

  const { data: pageResponse, isLoading, isError, error } = useQuery<PageResponse<EnrollmentStudent>, ApiError>({
    queryKey,
    queryFn: () => findAllEnrollments(Number(editionYear), {
      page: pageIndex,
      size: pageSize,
      q: debouncedFilter,
      sort,
  ...
  const { mutate: approveEnrollmentMutate, isPending: isApproving } = useMutation({
    ...
    onError: (error: ApiError) => showToast(error.message || "Erro ao aprovar inscrição", "error"),
  });
  ...
  const { mutate: refuseEnrollmentMutate, isPending: isRefusing } = useMutation({
    ...
    onError: (error: ApiError) => showToast(error.message || "Erro ao recusar inscrição", "error"),
  });
  ...
  const handleURLChange = useCallback((newParams: Record<string, string | number>) => {
    ...
    Object.entries(newParams).forEach(([key, value]) => {
      if (value !== undefined && value !== null) updatedParams.set(key, String(value));
  ...
  return {
    ...
    approveEnrollment: { mutate: approveEnrollmentMutate, isPending: isApproving },
