...
import { createNotice, type CreateNoticeRequest } from "../services/noticeService";
...
const avisoFormSchema = z.object({
  ...
  message: "Para envio via sistema, o destinatário deve ser 'Apenas Educadores'",
  path: ["recipient"], 
});

type AvisoFormSchema = z.infer<typeof avisoFormSchema>;

export const useDashboard = ({ setIsLoading }: UseDashboardParams) => {
  const {
    handleSubmit,
    control,
    formState: { errors },
  ...
  const onFormSubmit = async (data: AvisoFormSchema) => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : "Erro ao publicar aviso", "error");
    } finally {
      setIsLoading(false); 
    }
  };
