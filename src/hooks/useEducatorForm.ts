...
import { z } from "zod";
import { type CreateEducatorRequest, createEducator } from "../services/educatorService";
...
const EducatorFormSchema = z
    .object({
        ...
        cpf: z
            ...
            .regex(/^[0-9]{3}\.[0-9]{3}\.[0-9]{3}-[0-9]{2}$/, "O CPF deve estar no formato XXX.XXX.XXX-XX."),
        password: z
            .string()
...
export const useEducatorForm = () => {
    ...
    const { mutate, isPending } = useMutation({
        ...
        onSuccess: (response) => {
            showToast(response.message || "Educador cadastrado com sucesso!", "success");
            redirectTo("/login");
        },
        onError: (error) => {
