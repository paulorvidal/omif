...
export const useInstitutionTable = () => {
  ...
  const loadInstitutions = useCallback(async () => {
    setIsLoading(true);
    try {
      const result: PageResponse<FindAllInstitutionsResponse> = await findAllInstitutions(pageIndex, pageSize, debouncedFilter, sort);
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : "Erro ao deletar instituição", "error");
    } finally {
      setIsLoading(false);
    }
  };
