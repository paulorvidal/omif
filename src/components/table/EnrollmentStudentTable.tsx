...
const getStatusBadge = (status: string) => {
  switch (status) {
    case "APPROVED": return <Badge color="border-green-300 text-green-800 bg-green-50">Aprovada</Badge>;
    case "PENDING": return <Badge color="border-yellow-300 text-yellow-800 bg-yellow-50">Pendente</Badge>;
    case "REFUSED": return <Badge color="border-red-300 text-red-800 bg-red-50">Rejeitada</Badge>;
    default: return <Badge color="border-zinc-300 text-zinc-600">{status}</Badge>;
  }
};
...
export const EnrollmentStudentTable = ({ onCountChange, editionYear, isEditionActive }: Props) => {
  const {
    data,
    pageCount,
    pagination,
    ...
    return [
      ...
      columnHelper.display({
        ...
        cell: ({ row }) => {
          ...
          const isPendingAction = approveEnrollment.isPending || refuseEnrollment.isPending;

          const approveButton = <Button icon={<Check size={16} />} size="sm" onClick={() => approveEnrollment.mutate({ enrollmentId: id })} isLoading={isPendingAction}>Aprovar</Button>;
          const refuseButton = <Button destructive icon={<X size={16} />} size="sm" onClick={() => refuseEnrollment.mutate({ enrollmentId: id })} isLoading={isPendingAction}>Recusar</Button>;
          ...
          return (
            <div className="flex justify-end gap-2">
              {isEditionActive ? (
                (() => {
                  switch (status) {
                    case "PENDING": return <>{refuseButton}{approveButton}</>;
                    case "REFUSED": return approveButton;
                    case "APPROVED": return refuseButton;
                    default: return null;
                  }
                })()
  ...
  }, [isEditionActive, approveEnrollment.isPending, refuseEnrollment.isPending]);
        ...
        <div className="flex flex-col gap-4 p-4 md:gap-8 md:p-8">
          <div className="flex w-full flex-col items-center gap-4 md:flex-row">
            ...
            <div className="grid w-full grid-cols-1 gap-4 md:flex md:w-auto">
              <Button icon={<ListFilterPlus />} type="button" onClick={filterDialog.onOpen} outline>Filtros</Button>
            </div>
          </div>
          <GenericTable
            data={data}
