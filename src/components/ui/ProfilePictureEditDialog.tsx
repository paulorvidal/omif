...
export function ProfilePictureEditDialog(props: ProfilePictureEditDialogProps) {
    const { open, onClose, onSave, onDelete, isSaving, isDeleting, hasCurrentPicture } = props;
        ...
        <Dialog open={open} onClose={handleClose} maxWidth="xs" fullWidth>
            <DialogContent className="p-4">
                <div className="flex w-full items-center justify-between">
                    ...
                    <IconButton aria-label="close" onClick={handleClose} className="text-zinc-400">
                        <X size={24} />
                    </IconButton>
                </div>
                            ...
                            <div className="mt-6 flex w-full flex-col gap-2">
                                ...
                                {hasCurrentPicture && (
                                    <Button
                                        ...
                                        disabled={isDeleting}
                                    >
                                        {isDeleting ? <CircularProgress size={24} color="inherit" /> : <><Trash2 className="mr-2 h-4 w-4" /> Excluir Foto</>}
                                    </Button>
                                )}
                            </div>
                        </>
                    ...
                    {imgSrc && (
                        <div className="mt-6 flex w-full flex-col gap-2 sm:flex-row">
                            ...
                            <Button
                                ...
                                disabled={isSaving}
                            >
                                {isSaving ? <CircularProgress size={24} color="inherit" /> : <><Check className="mr-2 h-4 w-4" /> Salvar</>}
                            </Button>
                        </div>
                    )}
                </div>
