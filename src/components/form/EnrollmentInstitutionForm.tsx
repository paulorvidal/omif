...
const EnrollmentView = ({ editionYear }: { editionYear: string }) => {
  ...
  if (isLoadingStatus) {
    return (
      <div className="p-8 text-center">Verificando status da inscrição...</div>
    );
  }
...
const StatusDisplay = ({
  ...
  enrollmentData: EnrollmentStatusResponse;
}) => {
  const statusConfig = {
    APPROVED: {
      ...
      description:
        "Parabéns! Sua instituição está confirmada para participar da edição.",
    ...
    REFUSED: {
      ...
      description:
        "Houve um problema com sua inscrição. Por favor, entre em contato com o suporte para mais detalhes.",
      iconBg: "bg-red-500",
    ...
    PENDING: {
      ...
      description:
        "Sua inscrição foi recebida com sucesso e está aguardando a análise pela nossa equipe.",
      iconBg: "bg-amber-500",
  ...
  const currentStatus =
    statusConfig[enrollmentData.status as keyof typeof statusConfig] ||
