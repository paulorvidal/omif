...
export const Notice = ({ id, title, date }: NoticeProps) => {
  ...
  return (
    <>
      <div
        className="flex w-full min-w-full flex-1 items-center justify-between gap-4 rounded-md bg-zinc-50 p-4 duration-500 hover:bg-zinc-50/50 focus:bg-zinc-50/50 sm:min-w-96 md:p-8"
        onClick={() => setIsDetailsOpen(true)}
      >
        <div className="flex min-w-0 items-center gap-4">
          <Info className="size-8 text-blue-500" />
          <div className="flex flex-col justify-center">
            <h3 className="text-xl font-semibold break-all">{title}</h3>
